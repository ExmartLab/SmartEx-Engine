...
public class ExplanationController {
	...
	@Autowired
	CounterfactualExplanationService counterfactualExSer;
	 ...
	 * @param expgentype 		indicating type of explanation: If true, this
	...
	public ResponseEntity<String> getExplanation(@RequestParam(value = "min", defaultValue = "30") String min,
			...
			@RequestParam(value = "expgentype", defaultValue = "causal") String expgentype) {
		...
		if (expgentype.equals("contrastive")) {
			explanation = contrastiveExSer.getExplanation(minNumber, userId, device);
		} else if (expgentype.equals("causal")){
			explanation = causalExSer.getExplanation(minNumber, userId, device);
		} else {
			explanation = counterfactualExSer.getExplanation(minNumber, userId, device);
		}
